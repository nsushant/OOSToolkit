...
int main(int argc, char *argv[]) {
  ...
  std::vector<satellite_object> sats = build_walker_constellation(
      num_planes, num_satellites, relative_phase, altitude_km * 1000.0,
  ...
  sats.push_back(satellite_object::from_satellite_normal_to_ECI_coords(
      "service_1", svc, 500.0));
    ...
    if (step % 10 == 0) {
      ...
      for (size_t i = 0; i < sats.size(); i++) {
        ...
        */
        ...
        csv << t << "," << i << "," << sats[i].satname << "," << sats[i].r(0)
            << "," << sats[i].r(1) << "," << sats[i].r(2) << "," << sats[i].v(0)
            << "," << sats[i].v(1) << "," << sats[i].v(2) << "\n";
      }
    }
  ...
  std::vector<arma::vec> lambert_solutions = lambert_solver(r1, r2,
  tof, 3.986004418e14 , 0, 1);

  std::cout << "Number of solutions found: " << lambert_solutions.size() <<
  std::endl ;

  for (int sols=0 ; sols < lambert_solutions.size(); sols++){

  std::cout << "Solution number: " << sols+1 << std::endl;
  std::cout << std::setprecision(10) << "V1 (at service station) (m/s): " <<
  get_v1(lambert_solutions,sols) << std::endl; std::cout <<
  std::setprecision(10) << "V2 (at client) (m/s): " <<
  get_v2(lambert_solutions,sols) << std::endl;
  ...
  find_optimal_trajectory("sat_10", "service_1", 0.0, 60000.0, v1sol, v2sol,
  ...
  {depot_name,cl1_name,depot_name,cl2_name,depot_name,"sat_10",depot_name};


  const auto start = std::chrono::high_resolution_clock::now();
  schedule_struct init_schedule = create_schedule(deltaV_of_schedule_init,
  t_arrive, t_depart, sat_names_in_schedule,simfile);

  std::cout<<"Initial Schedule"<<std::endl;
  view_schedule(init_schedule);

  schedule_struct findopt_schedule =
  local_search_opt_schedule(deltaV_of_schedule_init, init_schedule, 100,
  simfile); const auto stop = std::chrono::high_resolution_clock::now(); auto
  ...
  */
  ...
  std::vector<std::string> sat_names_in_schedule = {
      depot_name, cl1_name, depot_name, cl2_name,
      depot_name, "sat_10", depot_name};
  ...
  schedule_struct init_schedule = create_schedule_lambert_only(
      ...
      simfile, service_time,deadline_array);
  ...
  std::vector<std::string> moves_for_local_search = {"moves_dt2"};

  std::vector<double> grid_move_dts = {1000};//100,  200,  300,  400,  500,  600,
