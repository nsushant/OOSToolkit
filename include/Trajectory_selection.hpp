...
void find_optimal_trajectory(std::string client_satname,std::string service_satname, double t_request ,arma::vec &v1sol, arma::vec &v2sol, arma::vec &r1sol, arma::vec &r2sol,std::string filename){

    DataFrame simfile(filename);
    ...
    for (int t_ser=0 ; t_ser < available_t_service.n_elem ; t_ser++){
        ...
        for(int t_cl = 0; t_cl < possible_tofs.n_elem ; t_cl++){
            ...
            for (int sol=0 ; sol < sols.size(); sol++){
                ...
                std::cout << "Solution number: " << sol+1 << std::endl;
                ...
                if (abs(deltaTotal) < DeltaVMinima){
